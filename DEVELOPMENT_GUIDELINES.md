...
A custom hook has one clear responsibility. `useSubscriptionActions` pre-refactor was 708 lines covering lifecycle actions (pause/resume/skip/cancel), retention offers, and plan changes all in one hook — a god hook in function form. Where it can be split along those natural boundaries without changing its call signature, do so; where a caller genuinely needs all three groups of actions together, composing three smaller hooks into one is fine — collapsing them back into one 700-line function is not.

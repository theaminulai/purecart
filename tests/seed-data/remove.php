<?php
/**
 * Standalone entry point for `composer test:remove`.
 *
 * Loads WordPress directly via wp-load.php so the teardown can run without
 * WP-CLI installed — this WAMP setup doesn't have `wp` on PATH. Prefer
 * `wp eval-file tests/seed-data/teardown.php` instead wherever WP-CLI *is*
...
if ( ! file_exists( $wp_load ) ) {
	...
	fwrite( STDERR, "Could not find wp-load.php at: {$wp_load}\nThis assumes the plugin lives at wp-content/plugins/<slug>/tests/seed-data/ — edit the dirname() depth in remove.php if your layout differs.\n" );
	exit( 1 );
}

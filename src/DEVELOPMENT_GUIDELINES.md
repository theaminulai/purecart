...
**Initial page loads use Suspense, not a manual `if (loading) return …` branch.** This app is on React 18 with a Redux Toolkit data layer (`createAsyncThunk`), neither of which suspends a component on its own — `shared/suspense/useSuspenseThunk.ts` bridges the two with the standard "throw a pending promise, read the resolved value" resource pattern (see `shared/suspense/createResource.ts`'s comment for why; `use()` isn't available until React 19). The pattern, used today in `modules/subscriptions/components/SubscriptionsPage.tsx`/`SubscriptionDetailPage.tsx`, `modules/updates/components/UpdatesPage.tsx`, and `modules/analytics/components/SubscriptionAnalyticsPage.tsx`:

```tsx
// inside the page component, unconditionally, near the top:
useSuspenseThunk( 'subscriptions', () => dispatch( loadSubscriptions() ).unwrap() );
```

**The resource cache is module-level (`shared/suspense/resourceCache.ts`), not component state — this isn't a style preference, it's load-bearing.** A `useState(() => createResource(...))` version of this was tried first and shipped, then reverted: it worked correctly on a cold page boot but silently failed to suspend (or re-fetch) on in-app navigation. The cause is subtle enough to write down so it doesn't get reintroduced: a component that suspends on its very first mount attempt has no prior committed fiber, so exactly how/whether React re-runs its earlier hooks (including that `useState` initializer) when retrying after the thrown promise resolves isn't something to rely on — and it behaves differently depending on what else is mid-render at the same time, which a cold boot and an in-app `navigate()` genuinely differ on. A plain `Map` keyed by a string, living outside React entirely, has none of that ambiguity: "has this key been requested before" is a fact about the module, not about any particular render attempt.

The `key` argument is what makes the "only suspend on the true first load" behavior work — every call site loading the *same* data must pass the *same* key (`'subscriptions'` for all three subscriptions-data call sites above) so they share one fetch and one cache entry; a module that's already loaded this page session renders instantly on revisit, no re-suspend. Wrap the route in `<Suspense fallback={<XPageSkeleton />}>` at the point it's rendered (see `app/router/AppRoutes.tsx`), not inside the page component itself.

If a module's primary load also has its own unconditional refetch-every-mount `useEffect` (as `modules/updates/components/UpdatesPage.tsx` does — it deliberately re-fetches every time you visit, not just once per session), don't call `useSuspenseThunk` alongside it unchanged — that double-dispatches the same thunk on the one render that triggers the Suspense fetch. Use the lower-level `getOrCreateResource(key, factory)` directly, which also returns `created` (true only on the call that actually created the cache entry), and guard the effect's own dispatch behind it — see `UpdatesPage.tsx` for the worked example.

Every module with a page-level Suspense boundary gets its own `<Name>PageSkeleton.tsx` next to the page it stands in for, built from the shared `shared/ui/Skeleton` atom (a pulsing placeholder rect) composed into that page's real layout — a shared generic spinner isn't enough. Match the real layout's actual widths, not just its shape: table/list skeletons in particular should use flexible (`flex: 1` / percentage) cell widths that stretch to fill the container like the real table does, not small fixed-pixel placeholders — a skeleton noticeably narrower than the content it's standing in for reads as broken, not loading. Export it from the module's `index.ts` alongside the real page, same as `SubscriptionsPageSkeleton` is exported next to `SubscriptionsPage`.

Where a module's primary load already had its own unconditional refetch-every-mount `useEffect` (as `modules/updates/components/UpdatesPage.tsx` did) rather than an idle-gated one, don't just add `useSuspenseThunk` alongside it unchanged — that double-dispatches the same thunk on the first mount. Guard the existing effect's dispatch behind whether the Suspense resource already triggered one (see that file for the worked example) so first-load-this-session goes through Suspense exactly once, and later remounts keep whatever the module already did.

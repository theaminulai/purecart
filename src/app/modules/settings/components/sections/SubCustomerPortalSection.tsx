 ...
 */
export function SubCustomerPortalSection( {
	settings,
	update,
	...
	return (
		<>
			<SettingsToggleField
				label="Allow customer self-pause"
				checked={ settings.allowSelfPause }
				onChange={ ( v ) => update( 'allowSelfPause', v ) }
			/>
			<SettingsToggleField
				label="Allow customer self-cancel"
				checked={ settings.allowSelfCancel }
				onChange={ ( v ) => update( 'allowSelfCancel', v ) }
			/>
			<SettingsToggleField
				label="Allow customer early renewal"
				checked={ settings.allowEarlyRenewal }
				onChange={ ( v ) => update( 'allowEarlyRenewal', v ) }
			/>
			<SettingsToggleField
				label="Allow customer to skip renewal"
				checked={ settings.allowSkipRenewal }
				onChange={ ( v ) => update( 'allowSkipRenewal', v ) }
			/>
			<SettingsField
				label="Max skips per billing year (0 = unlimited)"
				type="number"
				value={ settings.skipLimitPerYear }
				onChange={ ( v ) =>

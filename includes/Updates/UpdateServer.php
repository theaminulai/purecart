...
class UpdateServer {
    ...
    public function register_routes(): void {
        register_rest_route( PURECART_API_NAMESPACE, '/update-check', [
            ...
            'args'                => [
                ...
                'version' => [ 'required' => false, 'sanitize_callback' => 'sanitize_text_field', 'default' => '0.0.0' ],
            ],
        ] );

        register_rest_route( PURECART_API_NAMESPACE, '/changelog', [
            'methods'             => \WP_REST_Server::READABLE,
    ...
    public function update_check( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
        ...
        $products = get_posts( [
            'post_type'   => 'product',
            'meta_query'  => [ [ 'key' => '_purecart_plugin_slug', 'value' => $slug ] ], // phpcs:ignore WordPress.DB.SlowDBQuery
            'fields'      => 'ids',
            'numberposts' => 1,
        ] );

        if ( empty( $products ) ) {
            return new \WP_Error( 'purecart_plugin_not_found', __( 'Plugin not found.', 'purecart' ), [ 'status' => 404 ] );
        }
        ...
        if ( ! $latest ) {
            return new \WP_Error( 'purecart_not_found', __( 'No versions available.', 'purecart' ), [ 'status' => 404 ] );
        }
        ...
        return rest_ensure_response( [
            ...
            'sections'     => [ 'changelog' => wp_kses_post( $latest->changelog ?? '' ) ],
    ...
    public function get_changelog( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
        ...
        if ( empty( $products ) ) {
            return new \WP_Error( 'purecart_not_found', __( 'Plugin not found.', 'purecart' ), [ 'status' => 404 ] );
        }

        $versions = ( new VersionManager() )->get_all( (int) $products[0] );

        return rest_ensure_response( array_map( static fn( $v ) => [
            'version'     => $v->version,
